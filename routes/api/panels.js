  ...
  .get('/', (req, res) => {
    if (req.query.panelsArray) {
      if (req.query.panelsArray[0] === 'CHILDREN') {
            ...
            .then(panelsArray => {
              ...
              panelsArray.forEach(panel => {
                ...
                if (childrenOBJ.rootDesc[_id] === undefined && rootId === null) {
                  ...
                  if (childrenOBJ.rootDesc[rootId].depth < depth)
                    childrenOBJ.rootDesc[rootId].depth = depth;
        ...
        Panel.find({ _id: { $in: req.query.panelsArray } })
          .populate('authorId')
          .then(panelsArray => {
            const panelsToReturnPojo = {};
            panelsArray.forEach(panel => {
