...
router.patch('/like/:id', (req, res) => {
  ...
  Promise.all([
    ...
    Panel.findById(req.body.rootId).populate('authorId')
  ])
    .then(userAndpanel => {
      ...
      if (!user) {
        return res.status(400).json({
          currentUser: "current user id isn't saved to the database "
        });
      }
  ...
  User.findOne({ email }).then(user => {
    ...
    bcrypt.compare(password, user.password).then(isMatch => {
      if (isMatch) {
        ...
        jwt.sign(payload, keys.secretOrKey, { expiresIn: 3600 }, (_err, token) => {
          res.json({
            success: true,
            token: 'Bearer ' + token
          });
